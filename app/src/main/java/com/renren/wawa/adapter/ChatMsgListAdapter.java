...
import com.renren.wawa.view.CustomTextView;
...
public class ChatMsgListAdapter extends BaseAdapter implements AbsListView.OnScrollListener {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        String name = StringUtil.getLimitString(item.getSenderName(), 10);
        ...
        if (item.getType() != 0) {
            ...
            // 根据名称计算颜色
            holder.textItem.setBackgroundColor(context.getResources().getColor(R.color.transparent));
    ...
    public void playDisappearAnimator(int pos, View view) {
        ...
        if (firstVisable <= pos) {
            ...
            BBLog.d(TAG, "playDisappearAnimator->unexpect pos: " + pos + "/" + firstVisable);
        }
    }
        ...
        if (animatorIdx < MAXANIMATORCOUNT) {
            ...
            if (position < mAnimatorInfoList.size()) {
                ...
                if (during < 0) {   // 剩余时长小于0直接设置透明度为0并返回
                    ...
                    BBLog.v(TAG, "continueAnimator->already end animator:" + position + "/" + item.getContext() + "-" + during);
                    return;
                }
            }

            // 创建属性动画并播放
            BBLog.v(TAG, "continueAnimator->pos: " + position + "/" + listMessage.size() + ", alpha:" + startAlpha + ", dur:" + during);
            playViewAnimator(itemView, startAlpha, during);
        } else {
            BBLog.v(TAG, "continueAnimator->ignore pos: " + position + "/" + listMessage.size());
        }
    }
    ...
    private void playDisappearAnimator() {
        for (int i = 0; i < mListView.getChildCount(); i++) {
            ...
            if (null == itemView) {
                BBLog.w(TAG, "playDisappearAnimator->view not found: " + i + "/" + mListView.getCount());
                break;
            }
            ...
            if (position < mAnimatorInfoList.size()) {
                ...
                BBLog.e(TAG, "playDisappearAnimator->error: " + position + "/" + mAnimatorInfoList.size());
            }
    ...
    private void clearFinishItem() {
        ...
        while (mAnimatorInfoList.size() >= listMessage.size()) {
            BBLog.e(TAG, "clearFinishItem->error size: " + mAnimatorInfoList.size() + "/" + listMessage.size());
            if (mAnimatorInfoList.size() > 0) {
                mAnimatorInfoList.remove(0);
            } else {
                break;
    ...
    public void notifyDataSetChanged() {
        BBLog.v(TAG, "notifyDataSetChanged->scroll: " + mScrolling);
        if (mScrolling) {
//            滑动过程中不刷新
